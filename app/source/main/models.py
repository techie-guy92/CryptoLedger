...
class EntryPoint(models.Model):
    ...
    significance = models.CharField(
        max_length=10, choices=SIGNIFICANCE, verbose_name="Significance"
    )
    signal_notes = models.TextField(blank=True, null=True, verbose_name="Signal Notes")
    image = models.ImageField(
        upload_to=upload_to, blank=True, null=True, verbose_name="Image"
    )
...
class ExitPoint(models.Model):
    ...
    exit_2 = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="Exit Point 2"
    )
    exit_3 = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="Exit Point 3"
    )
    signal_notes = models.TextField(blank=True, null=True, verbose_name="Signal Notes")
    image = models.ImageField(
        upload_to=upload_to, blank=True, null=True, verbose_name="Image"
...
class BoughtCoin(models.Model):
    ...
    def fx_adjusted_profit(self):
        if self.usdt_rate_buy and self.usdt_rate_sell:
            rate_change = (
                self.usdt_rate_sell - self.usdt_rate_buy
            ) / self.usdt_rate_buy
            return round(rate_change * 100, 2)
        return None

    def save(self, *args, **kwargs):
        if self.usdt_rate_buy is None:
            rate = get_usdt_rate_sync()
            if rate is not None:
                self.usdt_rate_buy = rate
        self.full_clean()
        super().save(*args, **kwargs)
...
class Analyst(models.Model):
    ...
    image = models.ImageField(
        upload_to=upload_to, blank=True, null=True, verbose_name="Chart"
    )
...
class MostBoughtCoin(models.Model):
    ...
    volume_flow = models.CharField(
        max_length=20, choices=VOLUME_FLOW, default="1", verbose_name="Volume Flow"
    )
    source = models.CharField(
        max_length=20, choices=SOURCE, blank=True, null=True, verbose_name="Data Source"
    )
