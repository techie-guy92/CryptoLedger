...
class EntryPointAdmin(admin.ModelAdmin):
    ...
    def coin_price(self, obj):
        return format_html(
            f'<span class="live-coin-price" data-coin="{obj.coin}">Loading...</span>'
        )
...
class BoughtCoinAdmin(admin.ModelAdmin):
    list_display = [
        ...
        "holding_value",
        "total_cost_usdt",
        "total_cost_irt_display",
        "avg_net_cost_usdt",
        "avg_net_cost_irt_display",
        "usdt_rate_buy_display",
        "usdt_rate_sell_display",
        "total_earn_usdt",
        "total_earn_irt_display",
        "profit_display_usdt",
        "profit_display_irt",
        "bought_at",
        "sold_at",
    ...
    def profit_display_usdt(self, obj):
        if obj.profit_usdt is not None:
            ...
            return format_html(
                f"<span style='color: {color};'>{obj.profit_usdt:.2f}</span>"
            )
        return "-"
    ...
    def profit_display_irt(self, obj):
        if obj.profit_irt is not None:
            ...
            return format_html(
                f"<span style='color: {color};'>{obj.profit_irt:,.0f}</span>"
            )
        return "-"
